...
def process_chest(data, filter_freq=1.5):
    # data = calculate_acc_zero(data)
    data = filter_chest(data, filter_freq)
    ...
    for peak in stance_peaks:
        ...
        mins_after_stance = argrelextrema(p[range(peak, peak+14)], np.less)[0]
        ...
        ICs.append(IC)
        # Calculate FCs
        if len(mins_after_stance) > 0:
            # pick local min closest to stance
            FC = peak + mins_after_stance[0]
            FCs.append(FC)
...
def plot_vertical_component(data):
    ...
    # plt.plot(range(0, len(data)), filter_chest(np.linalg.norm(p, axis=1), 15))
    # print(p)
    # plt.plot(range(0, len(data)), filter_chest(p, 20))
    # # plt.plot(range(0, len(data)), np.linalg.norm(d-p, axis=1))
    # plt.plot(filter_chest(-data[:, 2] - np.mean(-data[:, 2] ), 10))
    # plt.plot(-data[:, 2] - np.mean(-data[:, 2]))
    # plt.plot(range(0, len(data)), filter_chest(h, 10))
    # plt.plot(range(0, len(data)), filter_chest(acc_zero_data - np.mean(acc_zero_data, axis=0), 15))
...
def process_chest_multiple(subjectStart, subjectEnd, activityTypes=["Walk"], saveFig=False):
    ...
    for subject_num in range(subjectStart, subjectEnd):
        ...
        for activity in activityTypes:
            ...
            for file in os.listdir(loaddir):
                ...
                plt.close("all")
                # if saveFig:
                #     title = file.split(".")[0] + " " + "chest" + " " + "Accelerometer As Scalar"
                #     plt.figure(1)
                #     plt.clf()
